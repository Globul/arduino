...
#define EVT_DOTEST      8   // used for testing
...
//  dispMsg("State = ST_FORWARD");
	...
	else
	{
			...
			if (startTime > 2)
			{
				...
				if (abs(distance-LastScan) < (double) distance/10)
				{
					speed = -20000;
					setMotor(IDL, speed);
					setMotor(IDR, speed);
					sprintf(buf, "360: %d", startTime);
					dispMsg(buf);
					state = 2;	// ended
					delay(500);
				}
...
//  CycleCount = ESP.getCycleCount();
  ...
  if (evt != EVT_NONE)
  {
	...
	LastScan = sonarScan();
	if (State != ST_MANUAL && State != ST_TEST)
	{
